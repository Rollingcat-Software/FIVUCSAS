...
- **Last verified**: 2026-05-07 — Late wave: **tenant pre-flight gate landed (api #86 + web #81)** so wrong-tenant users (e.g. gmail user on Marmara hosted-login) now reject at the password step with localized inline error instead of passing password+MFA only to fail at /oauth2/authorize. Also: web #80 (8 step components theme audit + MultiStepAuthFlow step counter), web #82 (NFC contradictory state cleared, 7 OAuth2 backend errors mapped to specific friendly messages, RFC 6749 §4.1.2.1 state echo on error paths, LoginMfaFlow step counter from step 1). New investigation doc `INVESTIGATION_VERIFY_FLOW_2026-05-07.md` (22 findings). Earlier same-day: six-lens investigation surfaced 10 P0s (`INVESTIGATION_MASTER_2026-05-07.md`); **7 shipped same-day** (api #81/#82/#83/#84/#85, bio #73/#74). Highlights: legacy `/2fa/verify-method` 2FA bypass closed (api #84), embedding-encryption-at-rest activated (bio #73 — was theater before), `WatchlistCheckHandler` profile-gated out of prod (api #81), `AccountLockedException` activated with 423 contract (api #82), OTP 5-strike counter + V58 (api #85), face confidence 0.7 fallback removed (api #83), `live_camera_analysis` fail-closed (bio #74). Earlier same-day: web #75/#76/#77/#78/#79 (USER-BUG-3/6, F8, hosted-login tenant-mismatch error, face-model loading UX), api #75/#79/#80 (face-search tenant scope, yaml smoke, F6 permitAll pin), bio #70/#72 (CI ubuntu-latest, face-search detect-then-crop). **api + bio container rebuilds STILL PENDING** on Hetzner — operator must diff deployed-sha..HEAD per `feedback_audit_delta_before_rebuild.md` before `docker compose build --no-cache`. Critical to deploy: api #84 (2FA bypass) + bio #73 (embedding decrypt). Previously: 2026-05-04 — Wave 1+2 quality batches (9 api PRs #63–#71 + 4 web PRs #67–#70) merged. See `ROADMAP_OPTIMIZED_2026-05-04.md` for the live open list and `CHANGELOG.md` for per-PR detail. Previously verified: 2026-04-30 — Z-wave + ops sweep deployed AND post-deploy UX polish landed. **Late-day Profile/Dashboard sweep**: tenant hardcode (`'Marmara University'` literal in DashboardPage.tsx → `user.tenantName`), date i18n leak in MyProfilePage (`format()` without `locale` → `dateFnsLocale(lang)`), section title rename ("Kayıtlı Biyometrik Yöntemler" → "Kayıtlı Doğrulama Yöntemleri" since auto-bound EMAIL_OTP+QR_CODE aren't biometric per `ManageEnrollmentService.ensureSessionBoundEnrollments`), "Kayıtlı Gün" → "Kayıtlı Gün Sayısı", and a helperText on Active Sessions explaining why it can differ from the Dashboard's USER_LOGIN audit count (sign-up auto-login mints a refresh token but emits USER_CREATED, BE-M5 multi-device design). **Biometric-puzzle visualization**: FacePuzzle now overlays the full 468-pt mesh + named contours (face oval cyan, eyes emerald, eyebrows yellow, lips orange, iris pink — mirrors the Python reference at `practice-and-test/biometric-demo-optimized/src/presentation/ui/drawing.py`); HandGesturePuzzle overlays the 21-pt hand skeleton with HAND_CONNECTIONS; both flash green on hold. DrawingUtils dynamically imported on first camera activation. 2026-04-29 — Z-wave (post-AUDIT_2026-04-28 follow-up) all shipped. Z1: refresh-token rotation family revocation (V50 + RFC 6749 §10.4 reuse-detection, Sec-P2 #6) + OAuth2 `HostedAuthorizeCompleteRequest` Bean Validation with RFC 6749 §5.2 error-shape adapter (Sec-P2 #7) + MFA `expiresAt` boundary alignment (Edge-P2 #6). Z2: VitePWA `navigateFallback` + `cleanupOutdatedCaches` (Edge-P2 #9) — kills stale-shell 404s after Hostinger deploys. Z3: biometric-processor `MAX_FILE_SIZE` guard wired before API-key auth. Z4: deploy-SHA image tagging (Ops-P2 #7), audit DRAFT PRs #32/#45 closed as superseded, CHANGELOG updated. Z5: public `/face-demo` page (7 face capabilities — detection, landmarks, head pose, client passive liveness, server anti-spoof, quality, embedding visualization). Sec-P2 #8 (audit-log HTML escape) deferred — log-file-only today. Yesterday: 6 morning + 7 afternoon fixes; FK-cascade incident lesson saved. Audit DRAFTs (api #32, web #45) closed in Z4.
