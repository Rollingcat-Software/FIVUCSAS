...
3. **Fixed the verify step counter (#13):** web-app **PR #202**. Ports the dashboard's #158 preflight-authoritative `flowTotalSteps` into `verify-app/LoginMfaFlow.tsx` so the denominator no longer jumps 1/2→2/3. Display-only, additive, reversible. `tsc` clean; verify-app suite green + a new regression test.
   - **MERGED + FULLY DEPLOYED 2026-06-03** (user chose "full ship" over the freeze hold): squash-merged to main (`9796d869`) → `app.fivucsas.com` dashboard auto-deployed via Hostinger (run 26882903664, success); `verify.fivucsas.com` Docker rebuilt to bundle `index-m1t1pf45.js` — container healthy, JS asset + `/health` both 200, framing-CSP / `Cache-Control` / `X-Frame-Options: DENY` headers preserved. The rebuild was safe because the uncommitted verify-widget infra changes (the `secure-headers-framable@file` middleware swap, the framing-CSP allowlist, the `no-cache` shell header, the `sync-assets.sh` localhost guard) were verified **already live** + the middleware is defined at `infra/traefik/config/dynamic.yml:157`. The fix is now live on both surfaces; the zero-deploy demo workaround is no longer needed.

**Update (after "full ship"):** #202 IS now deployed (above). The other "not changed" items still hold: no login/auth *logic* change (the fix is display-only), no changes to the genuinely-open backend items (#11/#15) or the on-device-only NFC bug (#7).
